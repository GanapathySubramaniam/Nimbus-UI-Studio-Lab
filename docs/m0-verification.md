...
The next hosted run cleared CodeQL and both platforms' type/test/build checks, but the fresh-install gate found two additional defects: Vite's short-path application root produced a Windows 403; a cold transform registered a watcher after Vite's parallel shutdown had closed it, keeping the CI process alive. The launcher now canonicalizes its application root and drains environments before closing Vite. The clean-install check always uses `CI=true` for its child, consumes HTTP bodies, logs bounded phases and verifies natural shutdown rather than relying on Vite's non-CI stdin-end exit fallback. The failing CI-mode cold start was reproduced locally before this lifecycle change and passed after it. Temporary watcher diagnostics were removed from product code.
